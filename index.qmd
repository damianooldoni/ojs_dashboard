...
// Primary: renders a pre-rendered PNG from docs/assets/plots/<species_slug>__<polygon_id>.png
// Fallback: falls back to Observable Plot bar chart from JSON data (or a plain message)
function createPopupContent(polygon_id, species_name) {
  ...
  // Build PNG path: assets/plots/<species_slug>__<polygon_id>.png
  const imgSrc = `assets/plots/${slugify(species_name)}__${polygon_id}.png`;
  ...
  popupDiv.appendChild(fallbackDiv);

  img.onerror = () => {
    // Hide broken image; show Observable Plot fallback or plain message
    ...
    if (plotData) {
      const plot = Plot.plot({
        ...
        marks: [
          ...
          })), { x: "month", y: "value", fill: "steelblue" }),
          Plot.ruleY([0])
        ]
      });
      fallbackDiv.appendChild(plot);
    } else {
      fallbackDiv.textContent = 'No plot data available for this selection.';
    }
  };
